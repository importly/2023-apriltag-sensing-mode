...
public class CoorSys {
     ...
     */
    // public CoorSys(NavX hdg, Encoder_Pwf leftLd, Encoder_Pwf rightLd, Encoder_Pwf
    // leftFl, Encoder_Pwf rightFl){
    // navX = hdg;
    // whlLdEnc_L = leftLd;
    // whlLdEnc_R = rightLd;
    // whlFlEnc_L = leftFl;
     ...
     */
    public void update() {
        // prstDist = (drvEnc_L.feet() + drvEnc_R.feet())/2; //Distance since last
        // reset.
        ...
        if (Math.abs(deltaD) > 0.2)
            deltaD = 0.0; // Skip this update if too large.
    ...
    // public double drvFeet() { return (whlLdEnc_R.feet() + whlFlEnc_L.feet()) /
    // 2.0; }
    ...
    /**
     * @return an array of the calculated X and Y coordinates on the field since the
     *         last reset.
     */
    public double[] get() {
